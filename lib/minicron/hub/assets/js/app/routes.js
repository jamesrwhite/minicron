...
Minicron.ExecutionsRoute = Ember.Route.extend({
  model: function() {
    var executions = this.store.all('execution');
    return executions.get('length') > 0 ? executions : this.store.find('execution');
  },
...
Minicron.JobsRoute = Ember.Route.extend({
  model: function() {
    var jobs = this.store.all('job');
    return jobs.get('length') > 0 ? jobs : this.store.find('job');
  },
...
Minicron.HostsRoute = Ember.Route.extend({
  model: function() {
    ...
    return hosts.get('length') > 0 ? hosts : this.store.find('host');
  },
